...
clobjs=[(70,70,70),(150,150,150),(230,230,230),(0,70,0),(0,150,0),(0,230,0),(70,0,0),(150,0,0),(230,0,0),(70,70,0),(150,150,0),(230,230,0)]
clm=(245,245,245)
...
def ccc(snakes,cubes,mis,tx,ty,objs,dtc,tac,nbobjs,nbv,mode,tmin):
  if time.time()-dtc >= tac:
    ...
    while len(objs)<nbobjs:
        ...
        objs.append([px,py,random.randint(0,11)])
    for s in snakes:
        if not s.perdu:
                ...
                if n==0:
                    ...
                    if tt:
                        ...
                        if mode == 0 or mode==1:
                            ...
                            cl=s.cube[0][2]
                            s.cubes=[]
                            px,py=random.choice(5,tx-5),random.randint(5,ty-5)
                    ...
                    for o in objs:
                        if c[0]==o[0] and c[1]==o[1]:
                            if o[2]==0: s.agr+=1
                                ...
                                for hh in range(1):
                                    if len(s.cubes)>3:
                                        c=s.cubes[len(s.cubes)-1]
                                        del(s.cubes[len(s.cubes)-1])
                                        cubes.append([c[0],c[1]])
                            elif o[2]==10:
                                for hh in range(3):
                                    if len(s.cubes)>3:
                                        c=s.cubes[len(s.cubes)-1]
...
def game(modecl,tx,ty,mode,nbb,nbj,dif,affbords,affquadr,tmin,keysp,tps,palierpoint,paliertaille,nbteams,teamsp):
    teams=[]
    bpx,bpy=rx(50),ry(50)
    tc=rx(750)/tx
    dtc=time.time()
    ...
    while encourg:
        affgame(snakes,cubes,mis,tx,ty,objs,affbords,affquadr)
        snakes,cubes,mis,tx,ty,objs,dtc,tac,nbv=ccc(snakes,cubes,mis,tx,ty,objs,dtc,tac,nbobjs,nbv,mode,tmin)
        ...
        elif mode==2: #time-game
            if tps<=0: encourg=False
        elif mode==3: #the first at big size
            for s in snakes:
                if len(scubes)>=paliertaille: encourg=True
        elif mode==4: #the first at a lot of points
            for s in snakes:
                if s.points>=palierpoints: encourg=True
        elif mode==5: #teams match
            lpp=0
            for t in range(len(teams)):
                if teams[t].points>teams[lpp].points: lpp=t
    lp1=0
    lp2=0
    lp3=0
    if len(snakes)>0:
